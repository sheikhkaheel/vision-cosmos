...
import { z } from "zod";
import { get_precise_ra_and_dec, get_ra_and_dec, getRaAndDec, move_to_ra_and_dec, move_precise_to_ra_and_dec, cancel_goto_movement, move_to_azm_and_alt, move_precise_to_azm_and_alt, get_azm_and_alt, get_precise_azm_and_alt, slew_azm_variable, slew_alt_variable, slew_variable, slew_azm_fixed, slew_alt_fixed, slew_fixed, slew_stop, get_location, set_location, get_time, set_time, get_tracking_mode, set_tracking_mode, sync_ra_dec, sync_precise_ra_dec, get_device_version, get_device_model, is_connected, is_aligned, is_goto_in_progress, } from "./utils/telescope.js";
...
server.tool("move_to_azm_and_alt", "Move telescope to specified Azm/Alt", {
...
server.tool("move_to_azm_and_alt_precise", "Move telescope to precise AZM/ALT", {
    azm: z.number().describe("Right Ascension in hours"),
    alt: z.number().describe("Declination in degrees"),
}, async ({ azm, alt }) => {
    try {
...
server.tool("get_current_azm_and_alt_from_telescope", "Get Telescope coordinates AZM/ALT", z.object({}).shape, async () => {
    try {
...
server.tool("get_current_precise_azm_and_alt_from_telescope", "Get Precise Telescope coordinates AZM/ALT", z.object({}).shape, async () => {
    try {
...
//? Slew Movement Tools
// Slew to Azm
server.tool("slew_to_azm", "Slew the telescope at a variable rate in azimuth", {
    rate: z
        .number()
        .describe("The variable slew rate in arcseconds/second, negative values are reverse"),
}, async ({ rate }) => {
    try {
...
// Slew to Alt
server.tool("slew_to_alt", "Slew the telescope at a variable rate in altitude", {
    rate: z
        .number()
        .describe("The variable slew rate in arcseconds/second, negative values are reverse"),
}, async ({ rate }) => {
...
// Slew to Azm/Alt
server.tool("slew_to_azm_and_alt", "Slew the telescope at a variable rate in azimuth and altitude simultaneously", {
    azm_rate: z
        .number()
        .describe("The variable slew rate in arcseconds/second for azimuth, negative values are reverse"),
    alt_rate: z
        .number()
        .describe("The variable slew rate in arcseconds/second for altitude, negative values are reverse"),
...
// Slew to Fixed Azm
server.tool("slew_to_fixed_azm", "Slew the telescope at a fixed rate in azimuth", {
    rate: z
        .number()
        .describe("The fixed slew rate to use [-9, 9] where 0 is stop and negative values are reverse")
        .max(9)
        .min(-9),
}, async ({ rate }) => {
    try {
...
// Slew to Fixed Alt
server.tool("slew_to_fixed_alt", "Slew the telescope at a fixed rate in altitude", {
    rate: z
        .number()
        .describe("The fixed slew rate to use [-9, 9] where 0 is stop and negative values are reverse")
        .max(9)
...
server.tool("slew_to_fixed_azm_and_alt", "Slew the telescope at a fixed rate in azimuth and altitude simultaneously", {
    azm_rate: z
        .number()
        .describe("The fixed slew rate to use in azimuth [-9, 9] where 0 is stop and negative values are reverse"),
    alt_rate: z
        .number()
        .describe("The fixed slew rate to use in altitude [-9, 9] where 0 is stop and negative values are reverse"),
}, async ({ azm_rate, alt_rate }) => {
    try {
...
// Stop Slew Movement
server.tool("stop_slew_movement", "Stops the telescope from slewing in both azimuth and altitude", {}, async () => {
    try {
...
server.tool("get_current_location", "Returns the current location of the telescope", z.object({}).shape, async () => {
    try {
...
server.tool("set_location", "Sets the location of the telescope", {
...
server.tool("get_current_time", "Returns the current time of the telescope", z.object({}).shape, async () => {
    try {
...
server.tool("set_time", "Sets the time of the telescope", {
...
server.tool("get_tracking_state", "Returns the current tracking mode", z.object({}).shape, async () => {
    try {
...
server.tool("set_tracking_state", "Sets the tracking state of the telescope", {
    tracking_mode: z
        .string()
...
server.tool("sync_ra_and_dec", "Syncs the telescope to the specified right ascension and declination position in degrees", {
...
server.tool("sync_precise_ra_and_dec", "Syncs the telescope to the specified right ascension and declination position in degrees with highest precision", {
...
server.tool("is_telescope_connected", "Checks if the telescope is connected", z.object({}).shape, async () => {
    try {
...
server.tool("is_telescope_aligned", "Checks if the telescope is aligned", z.object({}).shape, async () => {
    try {
...
server.tool("is_telescope_goto_in_progress", "Checks if the telescope is currently performing a goto operation", z.object({}).shape, async () => {
    try {
