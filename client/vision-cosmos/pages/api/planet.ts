  ...
  try {
    ...
    if (
      ...
      return res.status(400).json({
        ra: "n/a",
        dec: "n/a",
        error: "Invalid Celesial Body",
      });
    }

    // const hexRa = raToHex(ra);
    // const hexDec = decToHex(dec);
    // let telescopeResponse;
    // try {
    //   const telescopeResponse = await moveTelescope(hexRa, hexDec);
    //   console.log("Telescope Response ==>", telescopeResponse);
    // } catch (err: unknown) {
    //   if (err instanceof Error) console.log("Waiting Error =>", err.message);
    //   console.log("Err =>", err);
    //   telescopeResponse = err;
    // }

    console.log(typeof ra, typeof dec);
    ...
    console.log("Reponse from the flask ==>", json);

    res.status(200).json({ ra, dec });
    return;
  } catch (err: unknown) {
